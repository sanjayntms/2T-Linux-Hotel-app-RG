<?php
header("Content-Type: text/html; charset=UTF-8"); // ensure UTF-8
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == "checkin") {
        $sql = "INSERT INTO hotel_guests (name, room_number, phone, email, check_in) 
                VALUES ('$name', '$room', '$phone', '$email', NOW())";
        $msg = $conn->query($sql) ? "✅ Guest checked in!" : "❌ Error: " . $conn->error;
    } elseif ($action == "checkout") {
        $sql = "UPDATE hotel_guests 
                SET check_out=NOW() 
                WHERE name='$name' AND room_number='$room' AND check_out IS NULL";
        $msg = $conn->query($sql) ? "✅ Guest checked out!" : "❌ Error: " . $conn->error;
    }
}

$checked_in = $conn->query("SELECT name, room_number, phone, email, check_in 
                            FROM hotel_guests 
                            WHERE check_out IS NULL 
                            ORDER BY check_in DESC");
$checked_out = $conn->query("SELECT name, room_number, phone, email, check_in, check_out 
                             FROM hotel_guests 
                             WHERE check_out IS NOT NULL 
                             ORDER BY check_out DESC");

$occupancy_data = [];
$res = $conn->query("SELECT DATE(check_in) as date, COUNT(*) as count 
                     FROM hotel_guests 
                     WHERE check_in >= CURDATE() - INTERVAL 6 DAY 
                     GROUP BY DATE(check_in)");
while($row = $res->fetch_assoc()) { 
    $occupancy_data[$row['date']] = $row['count']; 
}

$search_results = null;
if (!empty($_GET['search'])) {
    $q = $conn->real_escape_string($_GET['search']);
    $search_results = $conn->query("SELECT * 
                                    FROM hotel_guests 
                                    WHERE name LIKE '%$q%' 
                                       OR phone LIKE '%$q%' 
                                       OR email LIKE '%$q%'");
...
<div class="container py-4">
    ...
    <form method="post" class="card p-3 mb-3">
        <div class="row mb-2">
            <div class="col">
                <input type="text" name="name" class="form-control" placeholder="Guest Name" required autocomplete="off" value="<?= htmlspecialchars($name) ?>">
            </div>
            <div class="col">
                <input type="text" name="room" class="form-control" placeholder="Room Number" required autocomplete="off" value="<?= htmlspecialchars($room) ?>">
            </div>
        </div>
        <div class="row mb-2">
    ...
    <form method="get" class="mb-3 text-center d-flex justify-content-center">
        <input type="text" name="search" class="form-control d-inline w-50" 
